$(function (){
    ...
    var resUrl = encodeURI('http://sport.72bike.com/activity/front/wxLogin?url='+window.userInfo.nowUrl);
            ...
            $.ajax({
                ...
                success:function (data){
		            if (data._status == true) {//判断该用户已登录
		                ...
		                //alert("你好,请重新登录!");
		                window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf509754d3d6886a6&redirect_uri="+resUrl+"&response_type=code&scope=snsapi_login&connect_redirect=1#wechat_redirect";//微信登录
		            }
                },
                error:function (){
                	//alert("载入失败!请重新刷新网页!");
    ...
    //投票活动分享固定分享信息
    $.fn.weShareTouPiao = function (shareData){
        $.ajax({
            url:window.userInfo.url+"activity/front/activity/shareInfo",
            type:"GET",
            data:shareData,
        ...
        function weixin(data,shareData){
            wx.ready(function(res) {
                wx.onMenuShareAppMessage({
                    title: "骑二无比最酷宝宝评选，滑步车大奖等您来拿！！！",
                    desc:"动动手指选出您心中最酷的宝宝吧。",
                    link: "http://sport.72bike.com/kids.html",
                    imgUrl: "http://sport.72bike.com/images/share-img.jpg",
                    trigger: function(res) {},
                    success: function(res) {
                        $.ajax({
                            url:window.userInfo.url+"activity/front/activity/share",
